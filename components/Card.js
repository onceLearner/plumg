    ...
    return (
        <div className="flex flex-col  space-y-4 w-48   " style={{ height: "70vh" }}>
            <div className={` relative w-60 transition-all duration-500 ease-in  bg-gray-400  cursor-pointer `} style={{ height: isHover ? '33vh' : ' 35vh' }}>
                ...
                <Image onMouseOut={() => setIsHover(!isHover)} src={data.imgUrl} layout="fill" objectFit="cover" onMouseOverCapture={() => setIsHover(true)} className=" transform transition-transform duration-500 ease-in hover:scale-110" />
            </div>
